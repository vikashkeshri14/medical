...
export default function SideBar() {
  ...
  useEffect(() => {
    ...
    if (!loginInfo || loginInfo == "undefined") {
      localStorage.clear();
      return navigate("/drugs");
    }
  });
  return (
    <div className="ltn__tab-menu-list mb-50">
      <div className="nav">
        <Link to="/account" className={pathname == "/account" && "active show"}>
          Dashboard <i className="fas fa-home" />
        </Link>
        <Link to="/profile" className={pathname == "/profile" && "active show"}>
          Profiles <i className="fas fa-user" />
        </Link>
        <Link to="/address" className={pathname == "/address" && "active show"}>
        ...
        <Link to="/orders" className={pathname == "/orders" && "active show"}>
